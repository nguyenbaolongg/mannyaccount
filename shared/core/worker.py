...
WORKSPACE_ROOT = os.path.dirname(SHARED_DIR)
...
def run_worker_process(account_id):
    ...
    # Voice ID: ưu tiên voice_id riêng của từng account, nếu không có thì dùng voice chung
        ...
        for vid_url in videos_to_process:
            ...
            try:
                ...
                if not is_valid_vietnamese(raw_content) and is_valid_vietnamese(raw_title):
                    ...
                    continue
                # Lấy tone từ row_data và tra cứu Voice ID
                tone = row_data.get("tone", "Trung tính")
                selected_voice_id = tts_voice_id
                settings_path = os.path.join(PROJECT_ROOT, "apps", "creator_facebook", "data", "global_voice_settings.json")
                if os.path.exists(settings_path):
                    try:
                        import json
                        with open(settings_path, "r", encoding="utf-8") as fs:
                            voice_config = json.load(fs)
                            tone_id = voice_config.get(tone, "").strip()
                            neutral_id = voice_config.get("Trung tính", "").strip()
                            if tone_id:
                                selected_voice_id = tone_id
                            elif neutral_id:
                                selected_voice_id = neutral_id
                    except: pass
                    
                ctx.logger.info(f"   🎵 Tone nhận diện: {tone} -> Chọn Voice ID: {selected_voice_id}")

                link_ct, local_content = handle_tts_and_update_sheet(tts_api_key, raw_content, selected_voice_id, row_data['row'], sheet_url, False, save_dir=ctx.temp_dir)
                local_title = None
                if row_data.get("title_text"):
                    _, local_title = handle_tts_and_update_sheet(tts_api_key, row_data["title_text"], selected_voice_id, row_data['row'], sheet_url, True, save_dir=ctx.temp_dir)
