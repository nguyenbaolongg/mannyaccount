...
def open_browser_for_login(profile_id, proxy=None):
    ...
    user_data_dir = os.path.join(project_root_dir, "tiktok_data", profile_id)

    # Tạo thư mục nếu chưa có (Dù UI đã tạo rồi nhưng check lại cho chắc)
    if not os.path.exists(user_data_dir):
        os.makedirs(user_data_dir)
    ...
    try:
        ...
        print("👉 Đăng nhập xong hãy đóng cửa sổ Chrome.")
        print("="*50)
